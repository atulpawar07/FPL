-- Supabase Forward Migration: 20261005000000_fix_gate3_rpc_payment_breakdown_and_bowling_style.sql
-- Target: Supabase Postgres DB
-- Application: Cricket Player Registration & Payment Web Application
-- Description: Gate 3-R1 STEP 1B Repair Player RPC payment fee breakdown and remove invalid ::bowling_style enum casts
--
-- Fix 1 (allocate_player_registration_v2):
--   Populate owner_fee_paise = 0 and player_fee_paise = COALESCE(v_registration_fee, 50000)
--   in payments INSERT to satisfy check_combined_fee_breakdown constraint.
--
-- Fix 2 (allocate_owner_registration_v3):
--   Remove invalid ::bowling_style enum casts on p_owner_bowling_style and p_icon_bowling_style.
--   The registered_bowling_style_snapshot column is TEXT.

-- ========================================================
-- 1. REPAIR allocate_player_registration_v2
-- ========================================================
...
BEGIN
  ...
  -- Insert Registration (Explicit casts for custom PostgreSQL enums)
  ...
  -- Insert Payment record (Explicitly setting owner_fee_paise = 0 and player_fee_paise = fee to satisfy check_combined_fee_breakdown)
...
-- ========================================================
-- 2. REPAIR allocate_owner_registration_v3
-- ========================================================
...
BEGIN
  ...
  -- 6. Create Owner Registration Record (Owner as Player #1) (Valid enum casts, registered_bowling_style_snapshot is TEXT)
  ...
  -- 8. Create Icon Registration Record (Icon as Player #2) (Valid enum casts, registered_bowling_style_snapshot is TEXT)
