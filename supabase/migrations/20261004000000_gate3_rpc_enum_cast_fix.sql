-- Supabase Forward Migration: 20261004000000_gate3_rpc_enum_cast_fix.sql
-- Target: Supabase Postgres DB
-- Application: Cricket Player Registration & Payment Web Application
-- Description: Gate 3-R1 STEP 1 Fix PostgreSQL custom enum type casting for RPC functions
--
-- Explicit Cast Mappings in Registrations Table:
-- 1. registered_role_snapshot            -> cricket_role
-- 2. registered_batting_style_snapshot     -> batting_style
-- 3. registered_bowling_style_snapshot     -> bowling_style
...
BEGIN
  ...
  -- 6. Create Owner Registration Record (Owner as Player #1) (Explicit enum casts)
  ...
  INSERT INTO registrations (
    ...
    CASE WHEN p_owner_bowling_style IS NOT NULL AND TRIM(p_owner_bowling_style) <> '' THEN p_owner_bowling_style::bowling_style ELSE NULL END,
  ...
  -- 8. Create Icon Registration Record (Icon as Player #2) (Explicit enum casts)
  ...
  INSERT INTO registrations (
    ...
    CASE WHEN p_icon_batting_style IS NOT NULL AND TRIM(p_icon_batting_style) <> '' THEN p_icon_batting_style::batting_style ELSE NULL END,
    CASE WHEN p_icon_bowling_style IS NOT NULL AND TRIM(p_icon_bowling_style) <> '' THEN p_icon_bowling_style::bowling_style ELSE NULL END,
    NOW(),
    NOW()
  )
