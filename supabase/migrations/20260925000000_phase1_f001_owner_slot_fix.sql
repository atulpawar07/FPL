-- Supabase Forward Migration: 20260925000000_phase1_f001_owner_slot_fix.sql
-- Target: Supabase Postgres DB
-- Application: Cricket Player Registration & Payment Web Application
-- Phase 1 Defect Fix (F-001): Sequential Slot Number Derivation via MAX(slot_number) + 1

-- Forward patch: Update allocate_owner_registration_v3 to calculate slot_number using COALESCE(MAX(slot_number), 0) + 1
-- This prevents slot collisions if an intermediate slot is removed/cancelled while higher slots exist.
