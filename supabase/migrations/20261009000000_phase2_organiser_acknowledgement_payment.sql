-- Supabase Forward Migration: 20261009000000_phase2_organiser_acknowledgement_payment.sql
-- Target: Supabase Postgres DB
-- Application: Cricket Player Registration & Payment Web Application
-- Description: Phase 2 Database Updates:
--   1. Add created_by_auth_id column to registrations and team_owners tables.
--   2. Update payments table check_payment_status and check_payment_method constraints.
--   3. Create allocate_owner_registration_v4 RPC supporting created_by_auth_id and payment_method.

-- 1. Add created_by_auth_id to registrations and team_owners
...
CREATE INDEX IF NOT EXISTS idx_registrations_created_by ON registrations(created_by_auth_id);
CREATE INDEX IF NOT EXISTS idx_team_owners_created_by ON team_owners(created_by_auth_id);

-- 2. Update Payment Status Constraint (Canonical 8 States)
-- Extend PostgreSQL Enum Type first before adding constraint or referencing value
ALTER TYPE payment_status ADD VALUE IF NOT EXISTS 'AWAITING_ORGANISER_ACKNOWLEDGEMENT';
...
-- 3. Update Payment Method Constraint (Canonical Production Methods)
...
ALTER TABLE payments ADD CONSTRAINT check_payment_method
  CHECK (
    payment_method::text IN (
      'UPI_QR',
      'UPI',
      'ACKNOWLEDGE_BY_ORGANISER'
    )
  );

-- 4. Create allocate_owner_registration_v4 RPC
