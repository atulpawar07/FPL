-- Supabase Forward Migration: 20261002000000_fix_remote_rpcs_status_and_slot_number.sql
-- Target: Supabase Postgres DB
-- Application: Cricket Player Registration & Payment Web Application
-- Description: Gate 2C-R1 Repair RPC column references and PL/pgSQL variable collisions

-- ========================================================
-- 1. FIX allocate_player_registration_v2
--    - Replace invalid `tournaments.status` with `tournaments.registration_open`
...
BEGIN
  ...
  -- Insert Payment record (payment_status column, not status)
...
--    - Fix PL/pgSQL variable collision with slot_number using explicit table aliases (t_owners.slot_number)
--    - Fix payments insert column (payment_status, remove invalid order_id column)
...
BEGIN
  ...
  -- 10. Create Combined Payment Record (payment_status column used)
  INSERT INTO payments (
    registration_id,
    team_owner_id,
    amount,
